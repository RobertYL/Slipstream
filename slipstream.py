...
class MainApplication(tk.Frame):
    def __init__(self, root):
        ...
        #Position Variable, Graph, and Main Frame
        ...
        #self.settingsMenu.add_command(label="Sensitivity",)
...
class Engine():
    ...
    def update_fileF(self):
        ...
        for f in self.__class__._files:
    ...
    def update_eleF(self):
        ...
        for i, e in enumerate(self.__class__._eles):
            ...
            self.__class__._eleT.insert(tk.END, "\t" + "P: " +
                str(self.__class__._points[e][0]) + "\n")
            self.__class__._eleT.insert(tk.END, "\t" + "V: " +
                str(self.__class__._points[e][1]) + "\n")
            self.__class__._eleT.insert(tk.END, "\t" + "C: " +
                str(100.0 * self.__class__._points[e][2]) + "%" + "\n")
    ...
    def submit_eles(self):
        ...
        for ele in self.__class__._eles:
            self.__class__._points[ele] = [[-1,-1],[-1,-1], -1.0]
        self.update_eleF()
        self.eleW.destroy()
    ...
    def analyze(self):
        ...
        for data in self.__class__._data:
            for ele in self.__class__._eles:
                self.points = self.find_peaks(data, ele)
                self.__class__._points[ele][0] = self.points[0]
                self.__class__._points[ele][1] = self.points[1]
    ...
    def composition(self):
        ...
        for i, ele in enumerate(self.__class__._eles):
            comp = np.append(comp, (self.__class__._points[ele][0][1] - self.__class__._points[ele][1][1]) / self.__class__._ref_ele_sens[self.__class__._sens.get()][ele])
            total = total + comp[i]
        ...
        for i, ele in enumerate(self.__class__._eles):
            self.__class__._points[ele][2] = comp[i] / total
